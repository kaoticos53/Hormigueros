...
Estado: **CERRADO — 7/7 rodajas HECHAS** (2026-09-15) · predecesores: [`fase5-2a-atta.md`](fase5-2a-atta.md)
...
| ^ | **Sonda de churn con señal de ARENA (2026-09-15)** | MISMO protocolo que la sonda anterior pero cada hijo evaluado en la arena REAL (banda 200–260 u, 2700 ticks, 3 trials, grid 96 — la escala del 428: misma procedencia de señal); élite pre-llena 64/64 con fitness de arena recalculado | **CON SEÑAL COMPARABLE EL TOPE CAE**: el 428 de exportación re-evaluado en el protocolo 2700-ticks da 109.1 (otro horizonte temporal = otra escala) y en la gen 6 un hijo lo supera (112.4) y en la gen 16 fija el nuevo tope (117.6, que resiste las 104 gens restantes) · churn TOTAL a la baja: de 64 originales solo sobrevive 1 (63 slots ganados por hijos evaluados en arena — 188 admisiones de 720 hijos, 26 %) · la media del élite SUBE monótonamente 84.4→108.6 (+29 %): la selección de arena pura REFINA, no solo reemplaza · especies 16→10 (el sharing + arena consolidan linajes que ganan) · estructura estable (33.0n/200.3c): con señal de fitness comparable, la forma ganadora es el denso y la variación estructural no basta sola para ganar — el fitness lo decide el comportamiento · lectura para la rodaja 7: el pool NEAT bajo arena real es COMPETITIVO (nuevo tope en 16 gens) y la élite se vuelve MERITOCRÁTICA total — el grafo NEAT es un superset del MLP que evoluciona sin penalización |
| ^ | **Sonda multi-semilla del churn de arena (2026-09-15)** | semillas 42/7/2024 × 60 gens × 6 hijos/gen — el mismo protocolo de la sonda anterior, para descartar artefacto de semilla | **PATRÓN GENERAL, no artefacto**: las 3 semillas reemplazan el tope temprano (gen 1–7 el primer avance) y siguen subiendo escalonadamente hasta gen 15–40 (tope final 113.2 / 120.0 / 115.2 sobre iniciales 109.1 / 113.9 / 110.1) · originales supervivientes 2–5/64 — el churn ~93 % es universal bajo señal de arena · media del élite sube siempre (106.9 / 110.6 / 106.0) · especies 6–9 sin colapso · lectura: la meritocracia de arena es ROBUSTA y el pool NEAT mejora el patrón oro de forma fiable; las cifras de referencia del cierre (rodaja 7) usan esta escala de arena 2700-ticks, no la de exportación |
| ^ | **HECHO (2026-09-15)** | **Canal F generalizado + inspector**: `NeatBrain.Graph` (descripción topológica cacheada en construcción — ids canónicos, profundidad por oculto, conns activas); canal F emite `"graph":{"n","h":[profundidades],"c"}` SOLO en mundos NEAT; canal A 13º campo OPCIONAL `brainShape` ("8h/200c", ausente en MLP — los streams v1 no cambian); `SimSnapshot.BrainShapeOf`; parser Unity `GraphView` + `AntPose.BrainShape`; `ActivationViewModel.RenderNeat` reconstruye los ids canónicos desde n/h/c y renderiza el grafo arbitrario; tarjeta de linaje con la línea `cerebro:` mostrando las formas del linaje (cerebros que CRECEN entre cuerpos visibles) | 11 tests: topología coherente con la conversión densa (33n/8h/200c), render NEAT ≠ render MLP, grafo crecido (2 capas de ocultos) renderiza, BrainShape vacío para MLP, hash invariante con los campos nuevos, parser tolerante (12 y 13 campos), linaje con formas. **Defecto latente corregido**: `RenderNeat` indexaba `HiddenDepth` por posición del nodo en el vector — válido en el denso (ocultos contiguos) pero falso en grafos crecidos donde Kahn intercala salidas antes que ocultos de id mayor; ahora el índice de ocultos es un contador propio. 378/378 tests, 5 pins intactos (los hashes no cambian: campos solo-NEAT ausentes en los mundos canónicos) |
...
| ^ | **HECHO (2026-09-15)** | **Cierre F5.2c**: `WorldSim.SeedPoolFromNeatGenomes` (fundadoras con cerebro de grafo clonado por hormiga, Genome=null — la evolución in-arena no alimenta al pool; nacimientos sobre pool denso FRÍO de 4 genomas, el régimen de un mundo recién fundado — Birth() sobre élite vacía lanzaría, defecto destapado por la primera partida); `GameScenario` acepta --seed-pool v1 O v2 (PeekFormatVersion decide el camino); CLI `--mode pretrain --neat` (NeatCurriculumTrainer + WriteNeatFile v2); 6º pin CI `check-neat-command.sh` (hash `021ed04f…`: invasión lasius+eciton con pool v2 de 64 genomas en colonia 0, 7200 ticks); fixture trackeado `tests/fixtures/pretrain-neat.antgenome` (v2, 64 genomas, warm-v2 semilla, tope de arena 1064.9 tras 4 gens/etapa, 11 especies); test de cierre en suite (pool→siembra→hash determinista→canal F con grafo en vivo) | criterio del plan §3 CUMPLIDO: invasión Eciton vs presa sembrada con pool NEAT PROPIO, hash determinista reproducido 2×, y el grafo del cerebro sembrado visible por el canal F (campo graph n/h/c) — la misma partida que fija el pin. 379/379 tests, 6 pins intactos |
